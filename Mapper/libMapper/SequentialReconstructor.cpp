...
#pragma omp parallel for num_threads(MAX_NUM_THREADS) collapse(2)
        for (size_t imgId1 = 0; imgId1 < imgMatches.size(); ++imgId1)
        {
            for (size_t imgId2 = 0; imgId2 < imgMatches.size(); ++imgId2)
            {
                ...
                if (imgId1 == imgId2 || std::find(imgMatches[imgId1].begin(), imgMatches[imgId1].end(), imgId2) == imgMatches[imgId1].end())
                {
                    continue;
                }
    ...
    /*
        Decomposes 3x3 essential matrix E into rotation and translation
    */
                                                       ...
                                                       bool initialCloud)
    {
        ...
        // 2. check that there are no nans and positive depth
    ...
    {
        // 1. add landmark reference to newly added features
        for (size_t pairIdx = 0; pairIdx < featureIds.size(); ++pairIdx)
        {
            ...
            // 2. check that unprojected feature has positive depth and low reprojection error and was
        ...
        // 3. new landmarks creation & triangulation
    ...
    */
                                              ...
                                              const std::string &outFolder)
    {
        ...
        std::string cloudPath = "../out_data/clouds/cloud_initial.ply";
